...
generateButton.addEventListener('click', async () => {
  ...
  // Reset state for export button and stored cards
  ...
  try {
    ...
    if (responseText) {
      ...
      if (flashcards.length > 0) {
        errorMessage.textContent = '';
        currentFlashcards = flashcards; // Store for export
...
exportButton.addEventListener('click', () => {
  if (currentFlashcards.length === 0) {
    return; // Should not be visible, but as a safeguard
  }

  const fileContent = currentFlashcards
  ...
  // Append, click, and remove for cross-browser support
